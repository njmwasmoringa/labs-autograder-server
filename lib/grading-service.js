        ...
        if (serviceState === "idle") {
            switch (message.action) {
                case "grade":
                    ...
                    async function workIt(submissions) {
                        wsClient.emit("grade", {
                            usercourse: `${userId}-any`,
                            payload: {
                                submissions,
                    ...
                    async function testAssignmentSubmissions(assignmentIndex = 0) {

                        console.log("Assignments: ", assignmentIds.length);
                        let assignmentId = assignmentIds[assignmentIndex];
                        ...
                        if (submissions.length > 0) {
                            await workIt(submissions);
                        }

                        if (assignmentIndex < assignmentIds.length) {
                            await testAssignmentSubmissions(assignmentIndex + 1);
                        }
