                    ...
                    if (lastMsg && lastMsg.includes("js_test_report")) {
                        try {
                            ...
                            if (report.failures.length === 0) {
                                ...
                                status = "incomplete";
                                comment = `Please fix the following issues and 
                                    consider re-submitting this lab\n\nx ${report.failures.map(r => r.fullTitle).join('\nx ')}\n`;
                            }
        ...
        if (serviceState === "idle") {
            switch (message.action) {
                case "grade":
                    ...
                    const assignmentId = instructions.assignments.splice(0, 1)[0];
                    ...
                    const workIt = () => {
                        ...
                        return Promise.allSettled(submissions.splice(0, 2).map(submission => gradeAssignment(message.course, submission)))
                            .then(() => {
                    ...
                    if (instructions.assignments.length > 0) {
                        doWork(instructions);
                        return;
                    }
            ...
            if (quuedProcess.length > 0) {
                doWork(quuedProcess.splice(0, 1));
            }
            else {
                setStatus("idle");
            }
